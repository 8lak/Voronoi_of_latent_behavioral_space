...
try:
    ...
    df = pd.read_csv("data_in_csv/Contact Information (Responses) - Form_Responses2.csv")
...
feature_columns = [col for col in behavioral_features]
df_action_space_behave = df[feature_columns].copy()


df_action_space_behave.to_csv("data_in_csv/action_space_behave.csv")
...
df_action_space_behave.dropna(inplace=True)

# --- 4. Normalization ---
scaler = MinMaxScaler()
A_total = scaler.fit_transform(df_action_space_behave)

# --- 5. Final Output ---
print("Phase I complete with advanced feature engineering.")
print(f"Final features used for model: {df_action_space_behave.columns.tolist()}")
print(f"Shape of the final feature matrix (S_total): {A_total.shape}")
...
np.savetxt("data_in_csv/A_total_features.csv", A_total, delimiter=",")
print("File saved successfully.")
np.savetxt("data_in_csv/action_space_names.csv", df_action_space_behave.columns, fmt='%s')
