...
S_TOTAL_FILE = "data_in_csv/action_space/A_total_features.csv"
FEATURE_NAMES_FILE = "data_in_csv/action_space/action_space_names.csv" # You'll need this file from Phase 1
...
se = SpectralEmbedding(n_components=2, n_neighbors=5, random_state=42)
...
all_centroids.to_csv("data_in_csv/action_space_means.csv")
...
for cluster_id, group in master_df.groupby('cluster_id'):
    color = 'black' if cluster_id == -1 else plt.cm.Spectral(cluster_id / (len(master_df['cluster_id'].unique())-1))
